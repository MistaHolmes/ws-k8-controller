"""
Generate plots for Experiment-E (KEDA Baseline).

Reads CSVs produced by parse_logs_experiment_e.py and saves PNG plots in
PROCESSED_DIR/plots/.

Produces:
  connections.png  - active_connections over time
  replicas.png     - spec replicas over time
  reconnections.png- reconnection rate over time (if non-zero)
  combined.png     - overlay of connections + replicas

Usage:
  RAW_DIR=... PROCESSED_DIR=... python plot_experiment_e.py
  or just: python plot_experiment_e.py  (uses default paths)
"""
...
for path, name in [(replicas_path, "replicas.csv"), (connections_path, "connections.csv")]:
    if not os.path.exists(path):
        raise RuntimeError(f"{name} not found in {PROCESSED_DIR}. Run parse_logs_experiment_e.py first.")
...
def draw_phases(ax):
    for ts, label in zip(phase_markers, phase_labels):
        ...
        ax.text(ts + 1, ax.get_ylim()[1] * 0.95, label,
                fontsize=6, color=_PHASE_COLORS.get(label, "#666666"),
...
# --------------------------------------------------
# Plot 3: Reconnection Rate (if non-zero)
# --------------------------------------------------
...
# --------------------------------------------------
# Plot 4: Combined overlay — Connections + Replicas
# --------------------------------------------------
...
ax1.set_title("Experiment-E: Connections vs Replicas (KEDA)", fontsize=11)
