...
approach we describe in the companion paper.
...
~\cite{casalicchio2019auto,lorido2014auto}.
...
Here's the core issue in one sentence: \textbf{CPU usage tells you nothing about
...
\item \textbf{An actual measured storm rate.} With Prometheus instrumentation in
  place, we measure a peak of 512~connections/second triggered by one single HPA
...
\item \textbf{Concrete numbers to benchmark against.} Every failure mode we
  describe comes with a number attached -- 233\% over-provisioning, a 512~conn/s
...
stops reflecting actual service demand. Casalicchio and Perciballi
~\cite{casalicchio2019auto} showed that choosing relative versus absolute metrics
...
et al.~\cite{lorido2014auto,lorido2016adaptation} lay out theoretical foundations
...
al.~\cite{rossi2020horizontal} look at HTTP-rate-based and combined CPU+traffic
scaling, but neither approach prevents connection drops, because both still treat
requests as transient events. The same goes for SLA-aware
approaches~\cite{biswas2017hybrid}, QoS-aware elasticity
frameworks~\cite{tsagkaropoulos2021severity}, predictive decision
makers~\cite{nikravesh2018genetic}, and scalability analysis
frameworks~\cite{alsaid2019scalability} -- all of them assume the scaling metric is
\textit{stateless across pod boundaries}. Our study targets exactly the
...
stateless workloads, and our own preliminary baselines reproduced their findings --
...
\caption{Overview of the standard Kubernetes Horizontal Pod Autoscaler (HPA) control architecture. The diagram was created by the authors based on the Kubernetes HPA architecture described by Nguyen et al.~\cite{nguyen2020hpa} and the official Kubernetes architecture.}
\label{fig:hpa_arch}
\end{figure*}
...
workload is across every running replica. For CPU-based scaling of stateless
...
\caption{Results from Experiment A establishing a correct-behavior baseline. Under monotonic WebSocket load where CPU is artificially coupled to session count via continuous pinging, the clean staircase ascent, flat stabilization plateau, and controlled descent to the minimum demonstrate that HPA achieves the theoretical ideal only when signal-to-demand correlation is perfect.}
...
\paragraph{Results.} HPA scaled up to an average peak of 10~replicas, which is
$3.33\times$ the connection-derived optimum of 3 ($\lceil285/100\rceil$, based on
roughly 285 active connections during the 60-second ramp-up). What's driving this
is the mismatch between the 90-second cycle and the 300-second window: each
...
confirms this holds across all 3 replications.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/experiment-b1-hpa-churn/multi_averaged.png}
\caption{Representative result from Experiment B1 demonstrating severe HPA over-provisioning and delayed recovery under cyclic load. HPA saturates at 10~replicas, a 233\% over-provision relative to the connection-derived optimum of 3~replicas, and requires $\sim$650~seconds to recover. Shaded bands show $\pm$1 SD across 3 replications.}
\label{fig:exp_b1_replicas}
\end{figure}

\noindent\textbf{Interpretation.} HPA simply can't tell the difference between a
LOW period where connections are being held (CPU low, but capacity is still
needed) and one where the pod is genuinely idle. The 233\% over-provisioning and
650-second recovery time add up to a real resource-efficiency cost if you ran this
in production.
...
reconnection rate came out to 512~connections/second (left panel of
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.28\textwidth]{figures/experiment-b2-hpa-churn-instrumented/multi_averaged_reconnections.png}\hfill
\includegraphics[width=0.28\textwidth]{figures/experiment-b2-hpa-churn-instrumented/multi_averaged_connections.png}\hfill
\includegraphics[width=0.28\textwidth]{figures/experiment-b2-hpa-churn-instrumented/multi_averaged_replicas.png}
\caption{Multi-run averages from Experiment B2-Instrumented demonstrating that HPA scale-down events directly trigger severe reconnection storms and connection instability. \textbf{Left:} Reconnection rate showing peak observed average rates of 512~connections/second perfectly synchronized with scale-down events. \textbf{Center:} Active connection count showing instability induced by simultaneous bursts, peaking at a mean of 767~($\pm$46). \textbf{Right:} HPA replica changes, confirming that each downward step causes the corresponding session disruptions.}
...
mean peak of 512~conn/s, with the active connection count varying widely across
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.42\textwidth]{figures/experiment-b3-hpa-idle-connections/connections.png}\hfill
\includegraphics[width=0.42\textwidth]{figures/experiment-b3-hpa-idle-connections/combined.png}
\caption{Representative result from Experiment B3 showing staircase session destruction following successive HPA scale-down events. \textbf{Left:} Active connection count exhibits permanent staircase destruction ($800{\rightarrow}771{\rightarrow}708{\rightarrow}644$). Connections are irrecoverable under \texttt{reconnect: false}. \textbf{Right:} Combined signal view demonstrating that as CPU utilization (red) drops to near zero due to idle connections, HPA scales down (green), causing the permanent destruction of active sessions (blue).}
...
Parameter tuning alone doesn't fix this, which is exactly why we explore a
different approach in the companion paper.
...
\begin{table*}[t]
\centering
\caption{Summary of results across the four experiments. Each experiment isolates
and quantifies a distinct aspect of HPA behavior under stateful WebSocket workloads.}
...
B1 (Cyclic) & Alternating HIGH/LOW activity & Reaches 10~replicas rapidly & 233\% over-provisioning; 650~s to recover \\
B2-Inst. (Measured) & Cyclic + Prometheus instrumentation & Reconnection storm per scale-down & Peak 512~conn/s; connection instability (767~$\pm$46 peak, range 732--819) \\
...
idle, and that drop happened before every scale-down decision. Put all three
...
-- and sometimes moves the exact opposite way.

The companion paper gives us a useful comparison point: a controller running at
...
to which signal HPA is watching, not the proportional control law itself -- give
...
not just its parameters, and we test it experimentally in the companion paper.
...
termination window. That's the direction we take in the companion paper.
...
look different on production infrastructure.
...
realistic HIGH/LOW activity cycle and produced 233\% over-provisioning (10 replicas
...
reconnection rate of 512~connections/second, and real instability in the active
...
The source code, Kubernetes manifests, Prometheus dashboards, experimental scripts,
datasets, and plotting utilities used in this study are publicly available at:
...
including Claude and ChatGPT, to assist with language refinement, grammar correction,
manuscript organization, and editorial improvements. The study conception, methodology,
software implementation, experimental design, data collection, analysis, interpretation
of results, and scientific conclusions were developed, verified, and approved by the
authors. The authors carefully reviewed and edited all AI-assisted content and take full
responsibility for the final manuscript.

\section*{Acknowledgements}

The authors are grateful to Dr.\ Rakesh Kumar Ranjan for his supervision,
guidance, and constructive feedback throughout this research. His input on
experimental design, measurement methodology, and how to structure the research
overall made a real difference to both the technical rigor and the presentation of
this work.

We'd also like to thank him for his encouragement throughout the development of
the experimental framework, and for helping us get access to the computational
resources we used to run the experiments and collect the results reported here.

Finally, we acknowledge the Department of Computer Science and Engineering, C V
Raman Global University, for providing the academic environment that made this
research possible.
