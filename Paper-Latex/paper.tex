...
This introduces a structural incompatibility: \textbf{CPU utilization cannot encode connection state fidelity}. A server sustaining 800 idle but active WebSocket sessions may exhibit near-zero CPU utilization, yet the destruction of a single pod hosting those sessions is immediately catastrophic to the affected clients. Conversely, brief CPU transients during connection establishment bear no relationship to sustained session capacity. Despite this fundamental mismatch, Kubernetes provides no native mechanism for connection-aware autoscaling, and HPA -- with its exclusive reliance on CPU and memory signals -- remains the default and recommended autoscaling primitive for all workload categories.
...
\item \textbf{Custom Kubernetes controller design.} We present the \texttt{StatefulAutoscaler}, a CRD-based Kubernetes controller implemented with the Kubebuilder~\cite{kubebuilder_docs} SDK, which scales exclusively on \texttt{sum(active\_connections)} queried from Prometheus and incorporates a sliding-window stabilization mechanism (\texttt{ScaleDownCooldownSeconds}) to absorb transient connection drops without triggering premature scale-down.

\item \textbf{Controlled experimental validation.} Under a two-cycle restorm simulation (Experiment C), we demonstrate exact replica targeting ($\lceil800/100\rceil = 8$, versus HPA's 15), complete pod preservation through a 90-second total disconnection gap, and zero connection loss across all scaling events -- conditions that are provably unachievable by any CPU-based autoscaler.
\end{enumerate}

The remainder of this paper is structured as follows. Section~\ref{sec:related} surveys related work. Section~\ref{sec:background} characterizes the baseline behavior of the two Kubernetes autoscaling pipelines under evaluation. Section~\ref{sec:problem} presents the evidence chain establishing HPA's failure for stateful workloads. Section~\ref{sec:controller} details the design and implementation of the custom controller. Section~\ref{sec:evaluation} presents experimental results. Section~\ref{sec:discussion} analyzes implications. Section~\ref{sec:limitations} discusses limitations and future directions. Section~\ref{sec:conclusion} concludes.
...
\textbf{Experimental Setup.} Load alternates between HIGH phases (800 connections, active CPU-generating pinging, 60-second duration) and LOW phases (connections held persistently idle, no pinging activity, 30-second duration). Crucially, during LOW phases the 800 client connections remain open; only the periodic ping messages cease, collapsing CPU utilization toward zero while connection state is preserved. Parameters were adjusted to \texttt{maxReplicas=15} to allow unconstrained scale-up; the HPA scale-down stabilization window was reduced to 60~seconds (	exttt{stabilizationWindowSeconds: 60}) to accelerate observable recovery behavior within the experiment duration.

\textbf{Results.} During HIGH phases, HPA exhausted \texttt{maxReplicas=15} within 99~seconds -- a factor of 1.875$\times$ the theoretically correct value of 8~replicas. Convergence to the minimum following LOW phases required approximately 650~seconds. The replica count timeline (Figure~\ref{fig:exp_b1_replicas}) exhibits the expected oscillation pattern between the replica ceiling and floor across successive cycles.
...
\textbf{Experimental Setup.} The load generator was reconfigured with \texttt{reconnect: false}. Once a client's connection is severed by pod termination, the client neither retries nor reconnects. CPU work was retained (\texttt{CPU\_WORK=1}) to maintain HPA activity, but the metric of interest is the cumulative active connection count.
...
The current implementation addresses the core architectural failure identified in this study. However, several limitations are acknowledged for future investigation.

\begin{enumerate}[leftmargin=*]
\item \textbf{Absence of graceful connection draining.} The \texttt{drain} specification field is present in the CRD but was not activated (\texttt{enabled: false}) during these experiments. In a production deployment, scale-down events must be preceded by a graceful migration of connections off the targeted pod -- a mechanism requiring coordination between the controller, the load balancer, and the application layer.

\item \textbf{Single point of metric dependency.} If the Prometheus instance becomes unavailable, the controller conservatively requeues without taking scaling action. While safe, this represents a potential availability risk. A production controller should support multiple metric source endpoints or fallback evaluation modes.

\item \textbf{Volatile stabilization state.} The sliding window is maintained exclusively in controller process memory. A controller restart following a crash resets the window history, potentially inducing premature scale-down during the period immediately following restart. Persisting window state to the CR status subresource would address this limitation.

\item \textbf{Absence of per-pod connection granularity.} The controller operates on the cluster-aggregate \texttt{sum(active\_connections)}, without visibility into per-pod connection distribution. Uneven distribution -- where some pods are near capacity and others are underutilized -- may lead to suboptimal scaling decisions. Incorporating per-pod connection queries would enable more precise density-based scaling.

\item \textbf{Window boundary edge case.} A brief replica dip to 5--6 was observed during early CYCLE 2 as stabilization window entries from before DROP 1 aged out while new CYCLE 2 connections had not yet fully populated the window. A configurable minimum hold-time following the most recent scale-up event would prevent this transient under-provisioning.
\end{enumerate}

\textbf{Planned enhancements} include: a graceful drain mechanism via a proxied \texttt{/drain} endpoint, per-pod metric query support, rate-based proactive scaling using \texttt{rate(new\_connections\_total[1m])} as a leading indicator, and persistent stabilization history maintained in the CR status field.

\paragraph{Edge Cases, Assumptions, and Mitigations}
We make several operational assumptions in our evaluation and identify corresponding mitigations:
\begin{itemize}[leftmargin=*]
\item \textbf{Instrumented servers assumed:} The controller requires an application-visible \texttt{active\_connections} gauge and (optionally) a \texttt{/drain} endpoint. Mitigation: parameterize the Prometheus query and drain path, and provide a lightweight exporter or sidecar adapter for uninstrumented brokers (e.g., Mosquitto) to bridge observability gaps.
\item \textbf{Prometheus availability and scrape fidelity:} Metric delays or missing samples can affect decisions. Mitigation: make the Prometheus endpoint and query configurable, tolerate transient query failures (safe requeue), and support per-pod queries or alternate metric backends as fallbacks.
\item \textbf{In-memory stabilization state:} Controller restarts currently lose the sliding-window history. Mitigation: persist stabilization history in the CR status (or an external store) to survive restarts and avoid premature scale-down.
\item \textbf{Uneven connection distribution:} Global replica counts assume reasonably balanced per-pod connections. Mitigation: collect per-pod connection gauges (or use session-affinity / client-side hashing) and, if necessary, incorporate imbalance-aware scaling heuristics.
\item \textbf{Broker diversity and generalizability:} Different brokers expose metrics and drain semantics differently. Mitigation: validate across brokers (WebSocket, MQTT, gRPC) and implement small adapter sidecars or exporters to normalize metrics and control endpoints.
\end{itemize}
...
This paper presented a systematic, empirically grounded investigation of Kubernetes Horizontal Pod Autoscaler behavior for stateful WebSocket workloads. Through a progressive evidence chain comprising four HPA characterization experiments (A, B1, B2-Instrumented, B3) and one controller validation experiment (C), we demonstrated that CPU-based HPA exhibits three distinct failure modes when applied to persistent-connection workloads: over-provisioning by 87.5\% under cyclic load (Experiment B1); reconnection storm induction at 1,400~connections/second during each scale-down event (Experiment B2-Instrumented); and permanent, staircase-pattern destruction of live idle connections (Experiment B3). A non-instrumented pilot run (B2-Extended LOW) served as the engineering stepping stone that motivated the full instrumentation of B2-Instrumented; it is not reported independently as it contributes no quantitative findings beyond those already measured with greater rigor in the instrumented variant. We then validated a practical mitigation (Experiment C): the \texttt{StatefulAutoscaler} controller, which uses \texttt{sum(active\_connections)} and a sliding-window stabilization mechanism to preserve pods through transient connection loss. These failure modes are architectural in nature and cannot be remediated through parameter tuning, reinforcing the findings of Nguyen et al.~\cite{nguyen2020hpa} on the fundamental behavioral constraints of the HPA proportional control loop.
...
These results validate that connection-aware autoscaling with sliding-window stabilization semantics is both necessary and sufficient for production-grade management of stateful WebSocket deployments in Kubernetes. The \texttt{StatefulAutoscaler} demonstrates that a controller of modest implementation complexity can eliminate the fundamental failure modes of CPU-based HPA for this growing class of workloads. As cloud-native architectures continue to adopt persistent-connection protocols -- from WebSocket to gRPC streaming, MQTT, and SSE -- connection-aware autoscaling represents an increasingly critical infrastructure requirement.
