...
\item \textbf{Systematic empirical evidence chain.} Through five progressive experiments (A, B1, B2-Instrumented, B3, C), we characterize and quantify the failure of CPU-based HPA for WebSocket workloads: from over-provisioning under cyclic load, to the measurement of peak reconnection storm rates (1,400~conn/s), to causal proof of permanent connection destruction via idle-connection scale-down and validation of the custom controller.
...
\textbf{Experimental Setup.} Load alternates between HIGH phases (800 connections, active CPU-generating pinging) and LOW phases (connections held persistently idle, no pinging activity). Parameters were adjusted to \texttt{maxReplicas=15} to allow unconstrained scale-up; scale-down stabilization was reduced to 60~seconds to accelerate observable behavior.
