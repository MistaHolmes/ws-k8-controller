...
\textbf{Results (5 independent runs).}
...
\textbf{Planned enhancements:} graceful drain via a proxied \texttt{/drain} endpoint; per-pod metric query support; rate-based proactive scaling; and persistent stabilization history in the CR status field.  We will also evaluate KEDA+HPA convergence under varying Prometheus scrape intervals and operator implementations, and explore per-cycle step-limiting or HPA-side safeguards to reduce reconnection blast radius.
...
These results validate that connection-aware autoscaling with sliding-window stabilization semantics is empirically effective for managing stateful WebSocket deployments in Kubernetes, eliminating all three failure modes documented in our evaluation \textit{under the synthetic two-cycle restorm workload evaluated here}. The \texttt{StatefulAutoscaler} demonstrates that a controller of modest implementation complexity can eliminate the fundamental failure modes of CPU-based HPA for this growing class of workloads. As cloud-native architectures continue to adopt persistent-connection protocols -- from WebSocket to gRPC streaming, MQTT, and SSE -- connection-aware autoscaling represents an increasingly critical infrastructure requirement.
